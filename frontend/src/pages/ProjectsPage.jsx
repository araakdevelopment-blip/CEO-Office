  ...
  return (
    <div data-testid="projects-page">
      <div className="flex items-center justify-between mb-7 flex-wrap gap-4">
        <div>
          <div className="text-xs uppercase tracking-[0.3em] text-yellow-500/80">إدارة المشاريع</div>
          <h1 className="font-heading text-4xl font-black text-slate-50 mt-2">مشاريع المجموعة</h1>
          <p className="text-slate-500 mt-1 text-sm">{filtered.length} مشروع • مفلتر حسب صلاحياتك</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="glass-card p-12 text-center text-slate-500">لا توجد مشاريع</div>
      ) : (
          ...
          {filtered.map(p => (
            <Link key={p.id} to={`/projects/${p.id}`} data-testid={`project-card-${p.id}`}
              ...
              <div className="mt-4 flex items-center justify-between text-[11px] text-slate-500">
                ...
                <span className="px-2 py-1 rounded bg-white/5">أولوية: {PRIORITY_LABELS[p.priority]}</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="glass-card p-6 max-w-lg w-full" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={submit} className="space-y-4">
              <input data-testid="form-project-name" required placeholder="اسم المشروع" value={form.name} onChange={e => setForm({...form, name: e.target.value})}
              ...
              <textarea placeholder="وصف موجز" value={form.description} onChange={e => setForm({...form, description: e.target.value})}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <input type="number" min={0} max={100} placeholder="نسبة الإنجاز" value={form.progress} onChange={e => setForm({...form, progress: e.target.value})}
                  className="px-4 py-2.5 rounded-lg bg-[#0a0d14]/80 border border-white/10 focus:outline-none text-sm"/>
                <input type="number" placeholder="الميزانية" value={form.budget} onChange={e => setForm({...form, budget: e.target.value})}
                  className="px-4 py-2.5 rounded-lg bg-[#0a0d14]/80 border border-white/10 focus:outline-none text-sm"/>
